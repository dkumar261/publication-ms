...
import com.publication.ms.support.rest.RestConstants;
...
public class SwaggerConfig {
    ...
    public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2)
				...
				.select()
					.apis(RequestHandlerSelectors.basePackage("com.dutech.myrdm.api"))
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder()
				.title("common-rdm Rest API")
				.description("Internal REST API for common-rdm")
				.version("1.0")
				.build();
    }
	...
	private List<Parameter> globalParams() {
		return Arrays.asList(
				new ParameterBuilder()	
					.name(RestConstants.PREAUTH_HEADER_LABEL)
